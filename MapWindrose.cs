...
namespace MapCreator
{
    public class MapWindrose : MapComponent, IXmlSerializable
    {
        ...
        private void DrawWindroseLines(SKCanvas canvas, bool fromCircle)
        {
            ...
            for (int i = 0; i < DirectionCount; i++)
            {
                ...
                if (fromCircle)
                {
                    ...
                    sp = MapDrawingMethods.PointOnCircle(InnerRadius, i * circleStep, new SKPoint(X, Y));
                }
                else
                {
                ...
                SKPoint ep = MapDrawingMethods.PointOnCircle(OuterRadius, i * circleStep, new SKPoint(X, Y));
        ...
        private void DrawFadedLine(SKCanvas canvas, SKPoint sp, SKPoint ep)
        {
            if (WindrosePaint != null)
            {
                SKPoint[] linePoints = MapDrawingMethods.GetPoints(254, sp, ep);
