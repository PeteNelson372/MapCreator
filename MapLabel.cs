...
namespace MapCreator
{
    public class MapLabel : MapComponent, IXmlSerializable
    {
        ...
        public Font LabelFont { get; set; } = new Font("Tahoma", 11.25F, FontStyle.Regular, GraphicsUnit.Point, 0);
            ...
            if (!string.IsNullOrEmpty(LabelText) && LabelPaint != null)
            {
                ...
                if (LabelPath == null || LabelPath.PointCount < 3)
                {
                    using (new SKAutoCanvasRestore(canvas))
                    {
                        ...
                        {
                            SKPaint glowPaint = MapPaintMethods.DeepCopyPaintObject(LabelPaint);
                            glowPaint.Color = LabelGlowColor.ToSKColor();
                            glowPaint.MaskFilter = SKMaskFilter.CreateBlur(SKBlurStyle.Outer, LabelGlowStrength);
                            canvas.DrawText(LabelText, point, glowPaint);
                        }

                        if (LabelOutlineWidth > 0)
                        {
                            SKPaint outlinePaint = MapPaintMethods.DeepCopyPaintObject(LabelPaint);
                ...
                else
                {
                    using (new SKAutoCanvasRestore(canvas))
                    {
                        ...
                        {
                            // draw box around label to show it is selected
                            canvas.DrawRect(boundsRect, MapLabelMethods.LABEL_SELECT_PAINT);
                        }
                    }
                }
            }
